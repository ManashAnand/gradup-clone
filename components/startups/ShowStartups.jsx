...
function Page({ selectedCity, selectedTitle, salaryExp, search,index ,jobs,loading,searchresults,setSearchresults}) {
  ...
  return (
    ...
    {/* {search.length >= 0 ? (
      ...
      {jobs.length>0 ? (
      <div style={{ backgroundColor: "" }} className="mt-10 arrange">
        {jobs.map((job) => (
...
export default function ShowStartups({ selectedCity, selectedTitle, salaryExp, search,index,searchresults,setSearchresults,setPage }) {
  const [jobs,setJobs] = useState([]);
  const [loading,setLoading]=useState(true)
  ...
  return (
    <div>
      ...
      {jobs.length>0?<div className="flex buttonpos ml-10">
