...
const Achievement = ({ achievementName, id }) => {
  ...
  const updateAchievement = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=achievement&action=update`,
        {
          method: 'POST',
          body: JSON.stringify({
            id: id,
  ...
  const deleteAchievement = async () => {
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=achievement&action=delete`,
        {
          method: 'POST',
          body: JSON.stringify({
            id: id,
    ...
    <div className='w-full'>
      <div className='flex flex-col'>
        <div className='px-3 py-3 rounded-2xl text-white bg-sky-700 resumedatabox text-center mt-6 mx-auto'>
          ...
          <Textarea
            css={{ height: 'fit-content', width: 'stretch', mt: '$4' }}
            initialValue={achievement.achievementName}
            onChange={(e) =>
              setAchievement({
                ...achievement,
