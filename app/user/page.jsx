...
export default function Profile() {
  ...
  const collegeList = [
    ...
    {
      value: 'Indian Statistical Institute Delhi',
      label: 'Indian Statistical Institute Delhi',
    },
    ...
    {
      value: 'Indian School of Mines Dhanbad',
      label: 'Indian School of Mines Dhanbad',
    },
    {
    ...
    {
      value: 'Indian Statistical Institute Bangalore',
      label: 'Indian Statistical Institute Bangalore',
    },
    ...
    { value: 'University of Kerala', label: 'University of Kerala' },
    {
      ...
      label: 'Tata Institute of Fundamental Research',
    },
    ...
    },
    {
      ...
      label: 'Delhi College of Engineering',
    },
    ...
    { value: 'FLAME University', label: 'FLAME University' },
    {
      ...
      label: 'O.P. Jindal Global University',
    },
    {
      value: 'SP Jain Institute of Management and Research',
      label: 'SP Jain Institute of Management and Research',
    },
    ...
    { value: 'Azim Premji University', label: 'Azim Premji University' },
    {
      ...
      label: "St. Xavier's College, Mumbai",
    },
    {
      value: 'Symbiosis Centre for Management Studies',
      label: 'Symbiosis Centre for Management Studies',
    },
    {
      value: 'Welingkar Institute of Management Development and Research',
      label: 'Welingkar Institute of Management Development and Research',
    },
    ...
    {
      value: 'Symbiosis Institute of Business Management',
      label: 'Symbiosis Institute of Business Management',
    },
    {
      value: 'Christ University, Bangalore',
      label: 'Christ University, Bangalore',
    },
    {
      value: 'Xavier Institute of Management, Bhubaneswar',
      label: 'Xavier Institute of Management, Bhubaneswar',
    },
    {
      value: 'KJ Somaiya Institute of Management',
      label: 'KJ Somaiya Institute of Management',
    },
    {
      value: 'Narsee Monjee Institute of Technology',
      label: 'Narsee Monjee Institute of Technology',
    },
    {
      value: 'Symbiosis Institute of Technology',
      label: 'Symbiosis Institute of Technology',
    },
    ...
    {
      value: 'Vellore Institute of Technology Bhopal',
      label: 'Vellore Institute of Technology Bhopal',
    },
    ...
    {
      value: 'JSS Academy of Technical Education Noida',
    ...
    {
      value: 'Shri Ramdeobaba College of Engineering and Management',
  ...
  if (error)
    return (
      <div>
        <p className='text-3xl animate-charcter justice textform mb-5 text-sky-500 '>
          Please Sign In to make your Profile !
        </p>
      </div>
    )
  if (!data)
  ...
  const addNewEducation = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=education&action=create`,
        {
          method: 'POST',
          body: JSON.stringify({
  ...
  const addNewProject = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=project&action=create`,
        {
          method: 'POST',
          body: JSON.stringify({
  ...
  const addNewExperience = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=experience&action=create`,
        {
          method: 'POST',
          body: JSON.stringify({
  ...
  const beAlumni = async (e) => {
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=alumni&action=update`,
        {
          method: 'POST',
          body: JSON.stringify({
            data: 'aaa',
  ...
  const addNewAchievement = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/user/${session?.user.id}/?type=achievement&action=create`,
        {
          method: 'POST',
          body: JSON.stringify({
  ...
  return (
    <div className='w-full mb-16'>
      <section className={`w-full ${styles['headingBox']} mb-6`}>
        ...
        >
          <Button className='py-2 px-5 bg-blue-400 text-white rounded-md hover:bg-blue-500 hover:text-white'>
            Your Applied Jobs
          </Button>
        </Link>
        <div>
          <Button
            className={`py-2 px-5 bg-blue-400 text-white rounded-md ${styles['margintop']} hover:bg-blue-500 ${styles['marginright']} hover:text-white`}
            disabled={data.education.length > 0 ? false : true}
            onClick={beAlumni}
          >
            Enter Alumni
          </Button>
        ...
        <Link href='/user/resume'>
          <Button
            className={`py-2 px-5 bg-blue-400 text-white rounded-md ${styles['margintop']} hover:bg-blue-500 hover:text-white`}
          >
            Download Resume
          </Button>
        </Link>
      ...
      <h2
        className={`text-4xl text-white ${styles['animate-charcter']} ${styles['textform']} mb-4`}
      >
        Create Your Resume
      </h2>
      <section className='  font-medium border-whitesmoke border rounded-xl text-left  bg-slate-900'>
        <div className='m-5'>
          <div className='w-2/4 flex flex-col m-auto'>
            <label
              className={`mb-2 ml-2 ${styles['textform']} text-white text-center text-sm`}
            >
              Name
            </label>
          ...
          <div className=''>
            <div id='education'>
              <span className='text-2xl font-semibold text-white my-auto'>
                Education
              </span>
              <p className='text-sm text-white'>
                (Add from top to bottom hierarchy)
              </p>
              <form onSubmit={addNewEducation} className='flex flex-col'>
                <div className={`${styles['flexBox']} w-full`}>
                  ...
                  {onefield ? (
                    <div className={`flex ${styles['item2']} flex-col mb-5`}>
                      ...
                      <Input
                        css={{ pr: '$4', width: 'stretch' }}
                        onChange={handlenewCollegename}
                ...
                <div className={`${styles['flexBox']} w-full `}>
                  ...
                  <div
                    className={`flex ${styles['item2']} flex-col ${styles['item1']} mb-5`}
                  >
                    <label
                      className={`mb-1 ml-2 mt-3 ${styles['textform']} text-white text-sm`}
                    >
                  ...
                  <div className={`flex flex-col ${styles['item2']} mb-5`}>
                    ...
                    {message2 && (
                      <p className='text-red-400 text-sm'>
                        *Leaving date must be greater than joining date
                      </p>
                    )}
                  </div>
                </div>
                <Link href='#education'>
                  <Button
                    disabled={disability4 && message2 ? true : false}
                    bordered
                    color='primary'
                    css={{
                      mx: 'auto',
          ...
          <div className=''>
            <div>
              ...
              <form onSubmit={addNewProject} className='flex flex-col'>
                ...
                <label className='mb-1 mt-3 ml-2 textform text-white text-sm'>
                  Description *
                </label>
                <Textarea
                  css={{ pr: '$4', mb: '$6' }}
                ...
                <Button
                  bordered
                  color='primary'
                  disabled={disability3 ? true : false}
                  css={{
                    mx: 'auto',
                    mt: '25px',
                    width: 'fit-content',
          ...
          <div className=''>
            <div>
              ...
              <p className='text-xs text-white'>
                (Enter all of your previous and current work experiences)
              </p>
              <form onSubmit={addNewExperience} className='flex flex-col'>
                ...
                <Button
                  className='hover:bg-blue-600 hover:text-white'
                  disabled={message3 && disability2 ? true : false}
                  bordered
                  color='primary'
                  css={{
                    mx: 'auto',
          ...
          <div className=' '>
            <div>
              ...
              <form onSubmit={addNewAchievement} className='flex flex-col'>
                ...
                <Button
                  className='hover:bg-blue-600 hover:text-white'
                  disabled={disability1 ? true : false}
                  bordered
                  color='primary'
                  css={{
                    mx: 'auto',
