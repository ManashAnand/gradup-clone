...
const page = () => {
    ...
    const { data, error } = useSWR( `/api/user/${session?.user.id}`, fetcher)
  ...
  return (
   ...
   {!submit? <div className=' w-full text-left'>
        ...
        <form onSubmit={createApplication} className='w-4/5 justify-center'>
            ...
            <button type='submit' disabled={data.education.length>0?false:true} style={{opacity:data.education.length>0?"1":"0.3"}} onClick={handleClick} className='bg-sky-400 mb-16 text-white rounded-md px-10 py-2 text-sm hover:bg-white hover:text-sky-400'>Submit</button>
        </form>
